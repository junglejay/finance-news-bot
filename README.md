...
这是一个由 GitHub Actions 驱动的无状态中文研究机器人。它在每个工作日北京时间 07:30 运行，收集商品、期货、原油、黄金、财务舞弊和内部控制相关的一手公告与权威公开订阅源；读取可公开访问的全文后，使用 OpenAI 兼容的 AI 网关逐篇生成中文深度阅读，再投递到钉钉机器人。
...
若要加入自己的权威公开订阅源，可设置 `EXTRA_RSS_FEEDS`：每行一个 `来源名称|https://rss-url`。若希望这些额外来源也参与“深度阅读”，再在 `EXTRA_ARTICLE_DOMAINS` 中用逗号列出其公开正文域名。只应加入无需登录、没有付费墙且你有权访问的域名。在 GitHub 上建议把这两个值设为仓库的 **Actions variables**，而不是 Secret。
...
工作流文件是 [.github/workflows/morning-brief.yml](.github/workflows/morning-brief.yml)。其中的 `30 23 * * 0-4` 是 UTC 时间，对应北京时间周一至周五 07:30；也可以在 GitHub 的 Actions 页面使用 **Run workflow** 手动触发。
